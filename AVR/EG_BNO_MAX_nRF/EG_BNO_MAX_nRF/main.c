...
void BNO_get_Euler(void)
{
	...
	i2c_rep_start(BNO055_ADDRESS + I2C_READ);	//Set device address and read mode
	N2_payload_TX[0] = i2c_readAck();		//Read Euler_H LSB
	N2_payload_TX[1] = i2c_readAck();		//Read Euler_H MSB
	N2_payload_TX[2] = i2c_readAck();		//Read Euler_R LSB
	N2_payload_TX[3] = i2c_readAck();		//Read Euler_R MSB
	N2_payload_TX[4] = i2c_readAck();		//Read Euler_P LSB
	N2_payload_TX[5] = i2c_readNak();		//Read Euler_P MSB
	i2c_stop();

	#if UART_DEBUG == 1
		//Combine the raw Euler bytes to word
		Euler_H_Raw = (N2_payload_TX[1] << 8) | (N2_payload_TX[0]);
		Euler_R_Raw = (N2_payload_TX[3] << 8) | (N2_payload_TX[2]);
		Euler_P_Raw = (N2_payload_TX[5] << 8) | (N2_payload_TX[4]);
