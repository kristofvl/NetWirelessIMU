...
uint8_t TX_address[5] = {0x01, 0x02, 0x03, 0x04, 0x05};
uint8_t RX_address[5] = {0x01, 0x02, 0x03, 0x04, 0x05};
...
void SPI_Init(void)
{
	...
	SPI_DDR |= _BV(MOSI) | _BV(SCLK) | _BV(CSN);
	SPI_DDR &= ~(_BV(MISO));
	
	//Enable SPI in Master mode with data-rate @F_CPU/16
...
** If an external clock is used this delay is reduced to 150us.
** CE should be held low/high for a minimum of 10us.
...
{	
	...
	_delay_us(10);
/*		
	//Reset the TX payload count
	uint8_t TX_pld_cnt = 0;
	
	for(TX_pld_cnt = 0; TX_pld_cnt < NRF24L01_PAYLOAD_CNT; TX_pld_cnt++)
	{
		//Send the payload data
		SPI_Write_Byte(TX_pld_data[TX_pld_cnt]);
		_delay_us(10);
	}
*/	
	SPI_Write_Byte(TX_pld_data);
	_delay_us(10);
